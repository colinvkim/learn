...
```text
-rwxr-xr--  1 colin  staff  1234  Jan 15 10:30  script.sh
```
...
The number `1` is the link count. `colin` is the owner. `staff` is the group. `1234` is the file size. The rest is the date and name.
...
sudo chown colin file.txt         # change owner to colin
```
...
<Callout title="PowerShell equivalent">
  PowerShell uses `$env:PATH` (semicolon-separated on Windows). Use `Get-Command` to find executables. Permission models differ significantly — Windows uses ACLs rather than Unix-style rwx. For most everyday work, PowerShell scripts require an execution policy change: `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser`.
</Callout>
...
<Callout title="Windows note">
  Windows does not have `sudo` in Command Prompt or PowerShell in the same way. To run as administrator, open the terminal "as Administrator." PowerShell 7+ includes `sudo` on recent Windows builds when enabled.
</Callout>
