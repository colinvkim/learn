...
<Callout title="PowerShell equivalent">
  Use `Get-Location` or the alias `pwd` (which PowerShell maps to `Get-Location`). The output looks the same.
</Callout>
...
<Callout title="PowerShell equivalent">
  Use `Get-ChildItem` or the alias `ls` (PowerShell maps `ls` to `Get-ChildItem`). Flags differ: `Get-ChildItem -Force` shows hidden items.
</Callout>
...
<Callout title="PowerShell and Command Prompt equivalents">
  Both PowerShell and Command Prompt also use `cd`. The behavior is the same. `cd ..` goes up one level. `cd` alone prints the current directory in Command Prompt but does nothing in PowerShell (use `Get-Location` instead).
</Callout>
...
```text
/Users/colin/Programming/learn/src
C:\Users\colin\Programming\learn\src
```
...
If you are in `/Users/colin/Programming/learn`, then `src/content/lessons` resolves to `/Users/colin/Programming/learn/src/content/lessons`.
