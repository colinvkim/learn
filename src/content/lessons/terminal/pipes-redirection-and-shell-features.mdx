...
<Quiz
  id="q-pipe-vs-redirect"
  question="What is the key difference between a pipe (|) and output redirection (>)?"
  options={[
    "A pipe sends output into another command; redirection writes output into a file",
    "A pipe writes only errors; redirection writes only normal output",
    "A pipe works only with text files; redirection works only with commands",
  ]}
  correctIndex={0}
  explanation="A pipe connects one command to another. `>` sends command output into a file instead."
/>
