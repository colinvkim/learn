...
<Quiz
  id="q-in-operator"
  question="What does the in operator help TypeScript do in a union?"
  options={[
    "Narrow the union by checking whether a property exists",
    "Convert every member of the union into an interface",
    "Validate that external JSON fully matches a type",
  ]}
  correctIndex={0}
  explanation='Checks like `"error" in result` let TypeScript narrow to the variants that contain that property.'
/>
