...
<Quiz
  id="q-discriminant-purpose"
  question="Why is a shared discriminant property so useful in a union?"
  options={[
    "Because checking that property lets TypeScript narrow safely to one variant",
    "Because it makes runtime validation unnecessary",
    "Because it removes the need for optional properties",
  ]}
  correctIndex={0}
  explanation="A discriminant like `status` or `kind` gives both you and TypeScript one reliable field to branch on."
/>
