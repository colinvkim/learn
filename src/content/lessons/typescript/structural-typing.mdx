...
<Quiz
  id="q-structural-shape"
  question="Why can a value of one named type often be assigned to another named type in TypeScript?"
  options={[
    "Because compatibility is based mainly on shape, not the type's name",
    "Because TypeScript ignores interfaces during assignment",
    "Because every object type is automatically compatible with every other object type",
  ]}
  correctIndex={0}
  explanation="TypeScript is structurally typed. If the required properties line up, the assignment is often allowed even when the type names differ."
/>
