...
<Quiz
  id="q-filter-choice"
  question="You want a new array containing only the users whose account is active. Which method is the best fit?"
  options={[
    "filter",
    "map",
    "reduce",
  ...
  correctIndex={0}
  explanation="`filter` keeps only elements that pass a test. `map` transforms every element, and `reduce` collapses the whole array into one accumulated result."
/>
