...
<Quiz
  ...
  options={[
    "The current async function until that promise settles",
    "The entire JavaScript runtime and every other task",
    "Only the network request, while the function continues running normally",
  ]}
  correctIndex={0}
  explanation="`await` pauses the current async function, not the whole runtime. Other work can still continue through the event loop."
/>
