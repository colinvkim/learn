...
<Quiz
  id="q-pnpm-store"
  question="What is the main storage idea behind pnpm?"
  options={[
    "Packages are stored in a shared content-addressable store and linked into projects",
    "Every project gets a completely separate full copy of every dependency",
    "Packages are loaded directly from the public registry each time a script runs",
  ]}
  correctIndex={0}
  explanation="pnpm avoids duplicating package contents across projects by reusing a shared store and linking files into each project's node_modules."
/>
