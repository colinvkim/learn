...
<Quiz
  id="q-workspaces-when"
  question="When do workspaces usually become worth the added complexity?"
  options={[
    "When one repository contains multiple related packages that need to stay coordinated",
    "Whenever you install your first dependency in a single-package project",
    "Only when you plan to publish every package publicly",
  ]}
  correctIndex={0}
  explanation="Workspaces are most useful when a repo holds several related packages that share dependencies or depend on each other."
/>
