...
<Quiz
  id="q-local-default"
  question="Which is the stronger default for a tool your project depends on, like ESLint or Vite?"
  options={[
    "Install it locally in the project",
    "Install it globally so every project shares one copy",
    "Avoid installing it and run it from the registry every time",
  ]}
  correctIndex={0}
  explanation="Project tools should usually be local dependencies so the whole team and CI use the same version."
/>
