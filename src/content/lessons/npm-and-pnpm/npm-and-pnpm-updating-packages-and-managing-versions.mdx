...
<Quiz
  id="q-version-ranges"
  question="Why do version ranges matter in package.json?"
  options={[
    "They affect which newer package versions the package manager is allowed to install",
    "They only control how dependencies are displayed on npmjs.com",
    "They decide whether a package is installed locally or globally",
  ]}
  correctIndex={0}
  explanation="A range like `^1.2.3` or `~1.2.3` tells the package manager what newer versions still count as acceptable for that dependency."
/>
