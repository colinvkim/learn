...
<note title="Peer dependencies are a contract">
  <p>
    When a package declares peer dependencies, it's saying "I need this package
    to be provided by the project using me." It's the project's responsibility
...
<warning title="Don't casually delete lockfiles">
  <p>
    Deleting and regenerating a lockfile can cause unexpected version changes.
    Do this only when necessary, and review the diff carefully to ensure it
    makes sense.
  </p>
</warning>
...
<Quiz
  id="q-peer-deps"
  question="What does a peer dependency warning usually mean?"
  options={[
    "A package expects your project to provide a compatible version of another package",
    "The package failed to download from the registry",
    "Your lockfile is corrupted and must always be deleted",
  ]}
  correctIndex={0}
  explanation="Peer dependencies describe compatibility expectations between packages, often for frameworks or plugins that must share the same host package version."
/>
