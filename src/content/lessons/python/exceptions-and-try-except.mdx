...
<Quiz
  id="q-else-clause"
  question="When does the else block of a try/except statement run?"
  options={[
  ...
  correctIndex={0}
  explanation="`else` is the success path for a `try` block. It runs only when the `try` section completed without raising an exception."
/>
