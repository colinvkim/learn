...
<Quiz
  id="q-generator-lazy"
  question="What is the main benefit of a generator compared with building a full list up front?"
  options={[
    "It produces values lazily, so work and memory use happen only as needed",
    "It always runs faster than every list-based approach",
    "It converts every loop into parallel execution",
  ]}
  correctIndex={0}
  explanation="Generators yield values one at a time. That makes them useful for large datasets, streams, and pipelines where you do not want every value in memory at once."
/>
