...
<Quiz
  id="q-html-decorative-image-alt"
  question="What should the `alt` attribute be for a purely decorative image that adds no information?"
  options={[
    "An empty string: `alt=\"\"`",
    "A short label like `alt=\"decorative image\"`",
    "Leave out the `alt` attribute entirely",
  ]}
  correctIndex={0}
  explanation="Decorative images should use an empty alt attribute so assistive technologies can ignore them deliberately."
/>
