...
<Note title="Do not use `section` as default wrapper">
  <p>
    <code>&lt;section&gt;</code> is not a more modern version of <code>&lt;div&gt;</code>. Use it when the content is a real thematic section of the document. If you only need a generic container for grouping, layout, or styling hooks, <code>&lt;div&gt;</code> is usually the better choice.
  </p>
</Note>
...
<Note title="`div` is neutral, not wrong">
  <p>
    Avoiding <code>&lt;div&gt;</code> entirely is not good HTML. The real goal is to use semantic elements when they fit, and use generic elements when no semantic meaning is being added.
  </p>
</Note>
...
<Quiz
  id="q-html-semantic-article"
  question="Which element best fits a self-contained event listing that could appear on its own page or in a feed?"
  options={[
    "<article>",
    "<section>",
    "<span>",
  ...
  correctIndex={0}
  explanation="`<article>` is meant for self-contained content that can stand on its own, such as a post, review, listing, or news item."
/>
