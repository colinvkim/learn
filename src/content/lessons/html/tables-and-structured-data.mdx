...
<Quiz
  id="q-html-table-purpose"
  question="Which situation is the best fit for a real HTML table?"
  options={[
    "A schedule with rows for dates and columns for session details",
    "A page layout with a sidebar on the left and content on the right",
    "A group of promotional cards on a homepage",
  ]}
  correctIndex={0}
  explanation="Tables are for true row-and-column data. Layouts and card groups should use other structural elements."
/>
