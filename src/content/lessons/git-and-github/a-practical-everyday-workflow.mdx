...
If you have configured commit signing (covered in the [signing lesson](/courses/git-and-github/git-signing-commits-with-ssh)), add `-S` to sign each commit:
...
<Quiz
  id="q-safe-next-step"
  question="You pulled the latest main and are about to start a new task. What is the strongest default next step?"
  options={[
    "Create and switch to a new branch for that task before making changes",
    "Work directly on main until the feature is almost done",
    "Open a pull request before writing any code",
  ]}
  correctIndex={0}
  explanation="This workflow treats a branch as the default workspace for each task. That keeps work isolated and makes review and merging cleaner."
/>
