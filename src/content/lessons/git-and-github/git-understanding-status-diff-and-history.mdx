...
<Quiz
  id="q-status-meaning"
  question="What does git status NOT show?"
  options={[
    "The actual line-by-line content of your changes",
    "Which branch you are on",
    "Which files are staged vs. untracked",
  ]}
  correctIndex={0}
  explanation="git status shows the repository state (branch, staged files, untracked files) but not the actual content of changes. For that, use git diff."
/>
...
<Quiz
  id="q-what-git-log-shows"
  question="What does git log --oneline display?"
  options={[
    "A compact list of recent commits with short IDs and messages",
    "The full diff of every commit in the repository",
    "A list of all branches and their current state",
  ]}
  correctIndex={0}
  explanation="git log --oneline gives a compact view: each commit as a short hash plus its message. For full diffs, use git log -p instead."
/>
